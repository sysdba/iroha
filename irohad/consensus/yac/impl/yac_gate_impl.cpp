  ...
  namespace consensus {
    namespace yac {
      ...
      YacGateImpl::YacGateImpl(
          ...
          uint64_t delay,
          network::ConsensusResultType result_cache)
      ...
      void YacGateImpl::vote(
        ...
        iroha::visit_in_place(block, [this, &hash](const auto &any_block) {
          ...
          result_cache_ =
              std::make_shared<shared_model::interface::BlockVariantType>(
                  any_block);
      ...
      YacGateImpl::on_commit() {
        return hash_gate_->on_commit().flat_map([this](auto commit_message) {
          ...
          return rxcpp::observable<>::create<
              shared_model::interface::BlockVariantType>([this, commit_message](
                                                             auto subscriber) {
      ...
      void YacGateImpl::copySignatures(const CommitMessage &commit) {
        for (const auto &vote : commit.votes) {
          ...
          iroha::visit_in_place(
              current_block_.second, [&sig](const auto &any_block) {
                any_block->addSignature(sig->signedData(), sig->publicKey());
              });
        }
      }
    }  // namespace yac
